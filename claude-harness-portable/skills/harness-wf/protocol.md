...
Supervisor 동작 (watchdog 완료 task-notification 또는 button dead-man's switch 주입 수신 시): `h2-log.sh last <h2dir>` 1회 Read → status 분기 — `stopped` / 워치독사망(button 주입) → **조용히 watchdog 1개 재스폰**(아래 dedup) [cap 은 self-respawn = Supervisor 미관여] / `phase_complete` → §12.5 Phase 완료 체크리스트 / `escalate|stuck` → 해당 SO diff read·중재 또는 Verifier 재가동(teammate-spawn.sh spawn-one). ⛔ **watchdog dedup**: 재스폰 직전 `.self-wake-ts` mtime 이 신선(<120s)이면 = 다른 watchdog 가 이미 alive → **재스폰 skip**(중복 sprawl 차단, 발견3 fix). 종료(S10) 시 `touch .watchdog-stop` → watchdog 다음 폴(≤60s) 안에 stopped 종료 + **dead-man's switch 해제** `bash ~/.claude/skills/harness-wf/lib/h2-registry.sh unregister "$(cd .harness && pwd)"` (button 감시 등록부에서 제거).
...
**12.5 Phase 완료 체크리스트 8항목 (phase-completion-checklist)**: watchdog 의 `phase_complete` 완료알림으로 Supervisor 가 기상하면(§6): (1)Healer 완료대기(미가동=정상) (2)파일소유권 정리 (3)**Phase Gate Review = security/quality/performance reviewer Agent 3 병렬 dispatch(model:sonnet, scope-disclaimer) + 요약 ev:gate_review 기록 + SR Gate-Review(mode G) — ⛔ 3-reviewer 가 G1~G4 커버 판단 시 SR mode G skip 허용하되 `ev:sr_skip {mode:"G",reason}` 명시 기록(묵시 skip 금지=발견1 drift). MUST-FIX 라우팅 = §7 격자(코드결함→Healer 9-step) + 전체 PASS 후 전환** (4)**SR Post-Review(mode A) = 매 phase 의무**(spawn-one MODE=A, KNOWLEDGE/PATTERN→promotion-log directive). ⛔ skip 불가 — phase_snapshot/S10 전 `ev:sr_review(A)` 존재 invariant(부재 시 진입 금지). (5)watchdog 재스폰(dedup)→IDLE (6)SR 지시서 §평가 ACCEPT/논의/DROP (7)harness.md Sufficiency Check 기입 + Sub-obj 체크박스 [x] (8)ev:phase_snapshot 기록.
